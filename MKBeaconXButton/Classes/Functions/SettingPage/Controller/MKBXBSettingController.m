...
@interface MKBXBSettingController ()<UITableViewDelegate,
...
@implementation MKBXBSettingController

- (void)dealloc {
    NSLog(@"MKBXBSettingController销毁");
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0 && indexPath.row == 0) {
        //Alarm event
        MKBXBAlarmEventController *vc = [[MKBXBAlarmEventController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 1) {
        //Dismiss alarm configuration
        MKBXBDismissConfigController *vc = [[MKBXBDismissConfigController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
    if (indexPath.section == 0 && indexPath.row == 2) {
        //Remote reminder
        MKBXBRemoteReminderController *vc = [[MKBXBRemoteReminderController alloc] init];
        [self.navigationController pushViewController:vc animated:YES];
        return;
    }
...
- (void)loadSubViews {
    ...
    [self.rightButton setImage:LOADICON(@"MKBeaconXButton", @"MKBXBSettingController", @"bxb_slotSaveIcon.png") forState:UIControlStateNormal];
...
- (MKBXBSettingPageModel *)dataModel {
    if (!_dataModel) {
        _dataModel = [[MKBXBSettingPageModel alloc] init];
    }
    return _dataModel;
}
